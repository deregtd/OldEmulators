...
cGen_GFX::cGen_GFX()
{
	...
	for (int i=0;i<256;i++)
	{
		PaletteGen[i] = (PaletteBaseGen[i] & 0x00FF00) | ((PaletteBaseGen[i] & 0xFF0000) >> 16) | ((PaletteBaseGen[i] & 0x0000FF) << 16);
			...
			case 2 :	//16-Bit Color
				{
					...
					COLORREF tpc = PaletteGen[i];

					if (CM555)
					{
						unsigned char Rtpc = (BYTE) ((tpc & 0x0000FF) >> 3);
						unsigned char Gtpc = (BYTE) (((tpc & 0x00FF00) >> 8) >> 3);
						unsigned char Btpc = (BYTE) (((tpc & 0xFF0000) >> 16) >> 3);

						FixedPalette[i] = (Rtpc << 10) | (Gtpc << 5) | (Btpc << 0);
					} else {
						unsigned char Rtpc = (BYTE) ((tpc & 0x0000FF) >> 3);
				...
				break;
			case 4 :	//32-Bit Color
				FixedPalette[i] = (PaletteGen[i] & 0x00FF00) | ((PaletteGen[i] & 0xFF0000) >> 16) | ((PaletteGen[i] & 0x0000FF) << 16);
				break;
			default :
...
void cGen_GFX::ReLoad(short Width, short Height, bool NewMode)
{
	if( D3DTEX != NULL )
		D3DTEX->Release();
	D3DTEX = NULL;
	...
	if (Mode3D)
	{
		HRESULT rst = pD3DD->CreateTexture(512, 256, 1, NULL, D3DFMT_P8, D3DPOOL_MANAGED, &D3DTEX);
	
		PALETTEENTRY TPAL[256];
		for (int i=0;i<256;i++)
		{
			TPAL[i].peRed = (BYTE) ((PaletteGen[i] & 0x0000FF) >> 0);
			TPAL[i].peGreen = (BYTE) ((PaletteGen[i] & 0x00FF00) >> 8);
			TPAL[i].peBlue = (BYTE) ((PaletteGen[i] & 0xFF0000) >> 16);
			TPAL[i].peFlags = 0;
		}
		...
		CUSTOMVERTEX g_Vertices[] =
		{
			...
			{  Width, Height, 0.5f, 1.0f, 0xffffffff, .625, .9375 },
...
void cGen_GFX::DrawScreen()
{
	...
	if (FPSCnt > FSkip)
	{
		...
		char tpo[80];
		if (FSkip == -1)
			sprintf(tpo,"NinthStar - Genesis - %i FPS (%i SlwDwn)",FPSnum,SlwDwn);
		else
			sprintf(tpo,"NinthStar - Genesis - %i FPS (%i FSkip)",FPSnum,FSkip);
