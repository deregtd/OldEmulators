...
void cNES_States::SaveState()
{
	...
	fwrite(&pPPU->Sprite->Sprite[0],1,0x100,out);
	for (int i=8;i<12;i++)
	...
	MSBSwapShort(tps);
	fwrite(&tps,1,2,out);
	fwrite(&pPPU->Sprite->SprAddr,1,1,out);
	...
	for (int i=0;i<NumBlocks;i++)
	{
		...
		if (strcmp(BlockSig,"BASR") == 0) {
			if (BlockHeader[0] == 1)
			{
				...
				fread(&pPPU->Sprite->Sprite[0],1,0x100,in);
				for (int ii=8;ii<12;ii++)
					fread(&pPPU->CHR_RAM[0x10 + ii - 8][0], 1, 0x400, in);
				fread(&pPPU->Palette[0][0],1,0x20,in);
				for (int ii=8;ii<12;ii++)
				{
					unsigned char tpa;
					fread(&tpa,1,1,in);
					pPPU->CHRPointer[ii] = (pPPU->CHR_RAM[0x10 + ii - 8] + (tpa << 10));
				}
				fread(&pPPU->VRAMAddr,1,2,in);
				MSBSwapShort(pPPU->VRAMAddr);
				fread(&pPPU->Sprite->SprAddr,1,1,in);
...
//				fread(&pSRAM->SRAMEnabled,1,1,in);
			...
			if (BlockHeader[0] == 1)
			{
				...
				{	
					...
					MSBSwapShort(tps);
					if (pPPU->Writable[i])
						Map_SetCHR_RAM1(ii,tps);
					else
						Map_SetCHR_ROM1(ii,tps);
				}
				unsigned char TPMD[0x80];
