...
void cA26_Debugger::DPrint(char * A, int B)
{
	if (Logging)
...
void cA26_Debugger::SetMode(int NewMode)
{
	Mode = NewMode;
	...
	if (gData->Main != 0)
	{
		TraceBMP = CreateCompatibleBitmap(TpHDC,A26WindowPoses[DoubleSize][DW_TRC_WIDTH]-8,A26WindowPoses[DoubleSize][DW_TRC_HEIGHT]-24);
		SelectObject(TraceDC,TraceBMP);
	}
	DestroyWindow(RegWnd);					//Registers
	if (gData->Main != 0)
	{
		RegBMP = CreateCompatibleBitmap(TpHDC,A26WindowPoses[DoubleSize][DW_REG_WIDTH]-8,A26WindowPoses[DoubleSize][DW_REG_HEIGHT]-24);
	...
	if (Mode >= 1) {	//Dumps
		//Dumps
		DumpWnd = CreateDialog(gData->hInst, (LPCTSTR) IDD_DUMPS, gData->MainWnd, A26DumpProc);
		SetWindowPos(DumpWnd,HWND_TOP,A26WindowPoses[DoubleSize][DW_DMP_LEFT],A26WindowPoses[DoubleSize][DW_DMP_TOP],A26WindowPoses[DoubleSize][DW_DMP_WIDTH],A26WindowPoses[DoubleSize][DW_DMP_HEIGHT],SWP_SHOWWINDOW);
	}
	if (Mode >= 3) {	// + Registers
		//Registers
		RegWnd = CreateDialog(gData->hInst, (LPCTSTR) IDD_REGISTERS, gData->MainWnd, A26RegProc);
		SetWindowPos(RegWnd,HWND_TOP,A26WindowPoses[DoubleSize][DW_REG_LEFT],A26WindowPoses[DoubleSize][DW_REG_TOP],A26WindowPoses[DoubleSize][DW_REG_WIDTH],A26WindowPoses[DoubleSize][DW_REG_HEIGHT],SWP_SHOWWINDOW);
		RegWDC = GetDC(RegWnd);
		//Trace Window
		TraceWnd = CreateDialog(gData->hInst, (LPCTSTR) IDD_TRACE, gData->MainWnd, A26TraceProc);
		SetWindowPos(TraceWnd,HWND_TOP,A26WindowPoses[DoubleSize][DW_TRC_LEFT],A26WindowPoses[DoubleSize][DW_TRC_TOP],A26WindowPoses[DoubleSize][DW_TRC_WIDTH],A26WindowPoses[DoubleSize][DW_TRC_HEIGHT],SWP_SHOWWINDOW);
...
void cA26_Debugger::StartLogging()
{
	Logging = true;
	LogFile = fopen("c:\\debug.txt","w");
}

void cA26_Debugger::StopLogging()
{
	fclose(LogFile);
	Logging = false;
}

unsigned char cA26_Debugger::TraceMem(unsigned short Addy)
{
	return(CPU->MemGet(Addy));			//LELELELE
}

void cA26_Debugger::DrawTraceLine(unsigned short Addy, short y)
{
	y -= 3;
	...
	switch (TraceAddyModeA26[TraceMem(Addy)])	
	{
	case ImmA : sprintf(tpc2,"%02X %02X     %s#$%02X", TraceMem(Addy), TraceMem(Addy + 1), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1)); break;
	case Abs : sprintf(tpc2,"%02X %02X %02X  %s$%04X", TraceMem(Addy), TraceMem(Addy + 1), TraceMem(Addy + 2), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1) | (TraceMem(Addy + 2) << 8)); break;
	case Zp : sprintf(tpc2,"%02X %02X     %s$%02X", TraceMem(Addy), TraceMem(Addy + 1), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1)); break;
	case Acc : sprintf(tpc2,"%02X        %sA", TraceMem(Addy), TraceArrA26[TraceMem(Addy)]); break;
	case Imp : sprintf(tpc2,"%02X        %s", TraceMem(Addy), TraceArrA26[TraceMem(Addy)]); break;
	case ERA : sprintf(tpc2,"%02X        %s", TraceMem(Addy), TraceArrA26[TraceMem(Addy)]); break;
	case Ind : sprintf(tpc2,"%02X %02X %02X  %s($%04X)", TraceMem(Addy), TraceMem(Addy + 1), TraceMem(Addy + 2), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1) | (TraceMem(Addy + 2) << 8)); break;
	case Absx : sprintf(tpc2,"%02X %02X %02X  %s$%04X,X", TraceMem(Addy), TraceMem(Addy + 1), TraceMem(Addy + 2), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1) | (TraceMem(Addy + 2) << 8)); break;
	case Absy : sprintf(tpc2,"%02X %02X %02X  %s$%04X,Y", TraceMem(Addy), TraceMem(Addy + 1), TraceMem(Addy + 2), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1) | (TraceMem(Addy + 2) << 8)); break;
	case Zpx : sprintf(tpc2,"%02X %02X     %s$%02X,X", TraceMem(Addy), TraceMem(Addy + 1), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1)); break;
	case Zpy : sprintf(tpc2,"%02X %02X     %s$%02X,Y", TraceMem(Addy), TraceMem(Addy + 1), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1)); break;
	case Idx : sprintf(tpc2,"%02X %02X     %s($%02X,X)", TraceMem(Addy), TraceMem(Addy + 1), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1)); break;
	case Idy : sprintf(tpc2,"%02X %02X     %s($%02X),Y", TraceMem(Addy), TraceMem(Addy + 1), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1)); break;
	case Rel :
		if (TraceMem(Addy + 1) & 0x80)
		{
			BYTE TpRelVal = ~TraceMem(Addy + 1) + 1;
			sprintf(tpc2,"%02X %02X     %s$%04X", TraceMem(Addy), TraceMem(Addy + 1), TraceArrA26[TraceMem(Addy)], Addy + 2 - TpRelVal);
		} else {
			sprintf(tpc2,"%02X %02X     %s$%04X", TraceMem(Addy), TraceMem(Addy + 1), TraceArrA26[TraceMem(Addy)], Addy + 2 + TraceMem(Addy + 1));
		}
		break;
	};
	...
	if (BreakP[Addy])
	{
		...
		trect.left = 0;
		trect.top = y+3;
		trect.right = A26WindowPoses[DoubleSize][DW_TRC_WIDTH];
...
void cA26_Debugger::Update()
{
	...
	if (Mode >= 3)
	{
		...
		if (gData->Main->Stop)	//Only while stepping
		{
			...
			trect.left = 0;
			trect.top = 0;
			trect.right = A26WindowPoses[DoubleSize][DW_REG_WIDTH];
			trect.bottom = A26WindowPoses[DoubleSize][DW_REG_HEIGHT];
			FillRect(RegDC, &trect, WBrush);

			trect.left = 0;
			trect.top = 0;
			trect.right = A26WindowPoses[DoubleSize][DW_TRC_WIDTH];
			trect.bottom = A26WindowPoses[DoubleSize][DW_TRC_HEIGHT];
			...
			BitBlt(RegWDC,0,0,A26WindowPoses[DoubleSize][DW_REG_WIDTH],A26WindowPoses[DoubleSize][DW_REG_HEIGHT],RegDC,0,0,SRCCOPY);
			...
			short MaxY = A26WindowPoses[DoubleSize][DW_TRC_HEIGHT]-24;
			...
			LineTo(TraceDC, A26WindowPoses[DoubleSize][DW_TRC_WIDTH], StartY);

			MoveToEx(TraceDC, 1, StartY+FontHeight, NULL);
			LineTo(TraceDC, A26WindowPoses[DoubleSize][DW_TRC_WIDTH], StartY+FontHeight);

			BitBlt(TraceWDC,0,0,A26WindowPoses[DoubleSize][DW_TRC_WIDTH],A26WindowPoses[DoubleSize][DW_TRC_HEIGHT],TraceDC,0,0,SRCCOPY);
		}
	}
}

void cA26_Debugger::AddInst()
{
	if (Logging)
	{
		...
		switch (TraceAddyModeA26[TraceMem(Addy)])	
		{
		case ImmA : sprintf(tpc2," %02X      %s#$%02X      ", TraceMem(Addy + 1), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1)); break;
		case Abs : sprintf(tpc2," %02X %02X   %s$%04X     ", TraceMem(Addy + 1), TraceMem(Addy + 2), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1) | (TraceMem(Addy + 2) << 8)); break;
		case Zp : sprintf(tpc2," %02X      %s$%02X       ", TraceMem(Addy + 1), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1)); break;
		case Acc : sprintf(tpc2,"         %sA         ", TraceArrA26[TraceMem(Addy)]); break;
		case Imp : sprintf(tpc2,"         %s          ", TraceArrA26[TraceMem(Addy)]); break;
		case ERA : sprintf(tpc2,"         %s          ", TraceArrA26[TraceMem(Addy)]); break;
		//Add Comments for Addy pointed to/Actual Addy on These
		case Ind : sprintf(tpc2," %02X %02X   %s($%04X)   ", TraceMem(Addy + 1), TraceMem(Addy + 2), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1) | (TraceMem(Addy + 2) << 8)); break;
		case Absx : sprintf(tpc2," %02X %02X   %s$%04X,X   ", TraceMem(Addy + 1), TraceMem(Addy + 2), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1) | (TraceMem(Addy + 2) << 8)); break;
		case Absy : sprintf(tpc2," %02X %02X   %s$%04X,Y   ", TraceMem(Addy + 1), TraceMem(Addy + 2), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1) | (TraceMem(Addy + 2) << 8)); break;
		case Zpx : sprintf(tpc2," %02X      %s$%02X,X    ", TraceMem(Addy + 1), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1)); break;
		case Zpy : sprintf(tpc2," %02X      %s$%02X,Y    ", TraceMem(Addy + 1), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1)); break;
		case Idx : sprintf(tpc2," %02X      %s($%02X,X)   ", TraceMem(Addy + 1), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1)); break;
		case Idy : sprintf(tpc2," %02X      %s($%02X),Y   ", TraceMem(Addy + 1), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1)); break;
		case Rel :
			if (TraceMem(Addy + 1) & 0x80)
			{
				BYTE TpRelVal = ~TraceMem(Addy + 1) + 1;
				sprintf(tpc2," %02X      %s-$%02X      ", TraceMem(Addy + 1), TraceArrA26[TraceMem(Addy)], TpRelVal);	//Is this Right?...
			} else {
				sprintf(tpc2," %02X      %s$%02X       ", TraceMem(Addy + 1), TraceArrA26[TraceMem(Addy)], TraceMem(Addy + 1));
			}
			break;
		};
		strcat(Out1, tpc2);
...
BOOL CALLBACK A26TraceProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		case WM_PAINT:
			BitBlt(Debugger->TraceWDC,0,0,A26WindowPoses[Debugger->DoubleSize][DW_TRC_WIDTH],A26WindowPoses[Debugger->DoubleSize][DW_TRC_HEIGHT],Debugger->TraceDC,0,0,SRCCOPY);
			break;
		case WM_KEYDOWN:
			switch (wParam)
			{
		...
		case WM_LBUTTONDOWN:
			{
				...
				if ((xPos > 0) && (xPos < A26WindowPoses[Debugger->DoubleSize][DW_TRC_WIDTH]) && (yPos > 0) && (yPos < A26WindowPoses[Debugger->DoubleSize][DW_TRC_HEIGHT]))
				{
...
BOOL CALLBACK A26RegProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_PAINT:
		BitBlt(Debugger->RegWDC,0,0,A26WindowPoses[Debugger->DoubleSize][DW_REG_WIDTH],A26WindowPoses[Debugger->DoubleSize][DW_REG_HEIGHT],Debugger->RegDC,0,0,SRCCOPY);
		break;
	}
	return false;
}
