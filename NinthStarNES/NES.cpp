...
//	CPU->Clockticks = &Clockticks;
    ...
    char tpo[40];
    sprintf_s(tpo,"NinthStar - NES - Stopped");
    ...
    MapperParam.Flags = 0;
    MapperParam.PRG_ROM_Size = 0x8000;
    MapperParam.CHR_ROM_Size = 0x2000;
    LoadMapperDLL(1);
    MapperInfo = MapperDllInfo->LoadMapper(0);
    PowerOn();
...
void cNES::PowerOn()
{
    ...
    ZeroMemory(&Sprite[0],sizeof(Sprite));
...
void cNES::OpenFile(char * FileName)
{
    ...
    strcat_s(SRAMFileName,FileName);
    SRAMFileName[strlen(SRAMFileName)-3] = 0x0;
    ZeroMemory(States->FileNameBase,sizeof(States->FileNameBase));
    strcat_s(States->FileNameBase, SRAMFileName);			//Save Filename Base to States Class
    char TPLSTFN[256];
    strcpy_s(TPLSTFN, States->FileNameBase);
    strcat_s(SRAMFileName,"sav");
    strcat_s(TPLSTFN,"lst");

    FILE *tpfile;
    fopen_s(&tpfile, TPLSTFN, "rt");
    if (tpfile)
    {
        ...
        if (ftell(tpfile) > 26)
        {
            ...
            while (!feof(tpfile))
            {
                ...
                sscanf_s(tpchr,"%4s   %04X",tpchr3,&TpAddy);
                
                if (TpAddy & 0x8000)
                {
                    TpAddy -= 0x8000;
                    sscanf_s(tpchr,"%4s   %4s %02X",tpchr3,tpchr3,&TpByte);
                    if ((TpByte < 0x100) & (TpByte > 0))		//Hack...
                    {
                        strcpy_s(Debugger->TraceComment[TpAddy],tpchr+24);
...
bool cNES::OpenFileUNIF(char *toopen)
{
    ...
    MapperInfo = MapperDllInfo->LoadBoard(BoardName);
    if (!MapperInfo)
    {
        MessageBox(gData->MainWnd, "Mapper Not Supported", "Error", MB_OK);
        return(false);
...
//				if (PPU->Xnum < 256)
                ...
                {
                    PPU->DiscoveredSprites = true;
                    PPU->StatusReg |= (PPU->SpriteCnt[PPU->SLnum+1] >= 8) << 5;
                }
            }
        }
        ...
        if ((CPU->Clockticks >= 1024) && (!HBYet))
        {
            ...
            if (MapperInfo->HBlank)
                MapperInfo->HBlank(PPU->SLnum, PPU->ControlReg[1]);
        }
...
void cNES::SetPPUMode(int PPUNum)
{
    ...
    if (OldPPU)
    {
        ...
        memcpy(PPU->Sprite, OldPPU->Sprite, sizeof(PPU->Sprite));
        PPU->SprAddr = OldPPU->SprAddr;
        for (int i=0;i<12;i++)
        {
            if ((OldPPU->CHRPointer[i] >= OldPPU->CHR[0]) && (OldPPU->CHRPointer[i] < (OldPPU->CHR[0] + sizeof(OldPPU->CHR))))
            { PPU->CHRPointer[i] = (unsigned char *) (OldPPU->CHRPointer[i] - OldPPU->CHR[0]); PPU->CHRPointer[i] += (DWORD) &PPU->CHR[0][0]; }
            else if ((OldPPU->CHRPointer[i] >= OldPPU->CHR_RAM[0]) && (OldPPU->CHRPointer[i] < (OldPPU->CHR_RAM[0] + sizeof(OldPPU->CHR_RAM))))
            { PPU->CHRPointer[i] = (unsigned char *) (OldPPU->CHRPointer[i] - OldPPU->CHR_RAM[0]); PPU->CHRPointer[i] += (DWORD) &PPU->CHR_RAM[0][0]; }
            else
                MessageBox(NULL, "FUCK", "UGH", MB_OK);
        }
...
//	HMENU hMenu = LoadMenu(gData->hInst, (LPCTSTR)IDM_NES_MENU);
    ...
    HMENU hMenu = GetMenu(gData->MainWnd);
...
//	Addy &= 0x7;
    ...
    case 0x0 :		//PPU Control 1
        if ((Val & 0x20) ^ (NES->PPU->ControlReg[0] & 0x20))
            NES->PPU->UpdateSprVector = 1;
    ...
    case 0x3 :
        ...
        break;
    case 0x4 :
        NES->PPU->Sprite[NES->PPU->SprAddr] = Val;
        NES->PPU->SprAddr++;
        NES->PPU->ppuLatch = Val;
    //Optim later
        NES->PPU->UpdateSprVector = 1;
...
//		NES->CPU->APU->Regs[0x13]=Val; NES->CPU->APU->Write4013(Val);
    ...
    case 0x014 :
        ...
        for (i=(Val << 8);i<((Val+1) << 8);i++)
            NES->PPU->Sprite[NES->PPU->SprAddr++] = NES->CPU->ReadHandler[i >> 12](i >> 12, i & 0xFFF);
        NES->PPU->UpdateSprVector = 1;
        break;
    case 0x015 :
...
void Map_SetWriteHandler(int Page, PWriteFunc New)
{	NES->CPU->WriteHandler[Page] = New; }

void Map_SetReadHandler(int Page, PReadFunc New)
{	NES->CPU->ReadHandler[Page] = New; }

PWriteFunc Map_GetWriteHandler(int Page)
{	return(NES->CPU->WriteHandler[Page]); }

PReadFunc Map_GetReadHandler(int Page)
...
unsigned char *	Map_GetPRG_Ptr4(int Where)
{
...
unsigned char *	Map_GetCHR_Ptr1(int Where)
{
...
void	Map_StatusOut(char *ToSay)
{
}

int		Map_GetMenuRoot()
{
    return((int) GetMenu(gData->MainWnd));
}

int		Map_AddMenuItem (int root, char *text, int cmd, int parm1, int parm2, int parm3, int mode)
{
    return(0);
}
...
void cNES::LoadMapperDLL(int Num)
{
    ...
    if (Num == 1)	//iNES
    {
        ...
        TDLLInfo * (*TPF)(int) = (TDLLInfo * (*)(int))GetProcAddress(MapperDllInst,"Load_DLL");
    ...
    MapperParam.StatusOut = &Map_StatusOut;
    MapperParam.AddMenuItem = &Map_AddMenuItem;
    MapperParam.GetMenuRoot = &Map_GetMenuRoot;
    MapperParam.MMC5_UpdateAttributeCache = &Map_MMC5_UpdateAttributeCache;
    MapperParam.IRQ = &Map_IRQ;
}
