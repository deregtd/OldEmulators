...
//	c2A03 *pC = CPU;
	char tpc[40] = { 0 };
	sprintf_s(tpc, "%04X  ", Addy);				//PC

	if ((Addy & 0x8000) && (TraceComment[Addy-0x8000][0]))
	{
		strcat_s(tpc, TraceComment[Addy-0x8000]);
	} else {
		char tpc2[30] = { 0 };						//Add Addy Mode Stuff

		switch (TraceAddyMode[TraceMem(Addy)])	
		{
		case ImmA : sprintf_s(tpc2,"%02X %02X     %s#$%02X", TraceMem(Addy), TraceMem(Addy + 1), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1)); break;
		case Abs : sprintf_s(tpc2,"%02X %02X %02X  %s$%04X", TraceMem(Addy), TraceMem(Addy + 1), TraceMem(Addy + 2), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1) | (TraceMem(Addy + 2) << 8)); break;
		case Zp : sprintf_s(tpc2,"%02X %02X     %s$%02X", TraceMem(Addy), TraceMem(Addy + 1), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1)); break;
		case Acc : sprintf_s(tpc2,"%02X        %sA", TraceMem(Addy), TraceArr[TraceMem(Addy)]); break;
		case Imp : sprintf_s(tpc2,"%02X        %s", TraceMem(Addy), TraceArr[TraceMem(Addy)]); break;
		case ERA : sprintf_s(tpc2,"%02X        %s", TraceMem(Addy), TraceArr[TraceMem(Addy)]); break;
		case Ind : sprintf_s(tpc2,"%02X %02X %02X  %s($%04X)", TraceMem(Addy), TraceMem(Addy + 1), TraceMem(Addy + 2), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1) | (TraceMem(Addy + 2) << 8)); break;
		case Absx : sprintf_s(tpc2,"%02X %02X %02X  %s$%04X,X", TraceMem(Addy), TraceMem(Addy + 1), TraceMem(Addy + 2), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1) | (TraceMem(Addy + 2) << 8)); break;
		case Absy : sprintf_s(tpc2,"%02X %02X %02X  %s$%04X,Y", TraceMem(Addy), TraceMem(Addy + 1), TraceMem(Addy + 2), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1) | (TraceMem(Addy + 2) << 8)); break;
		case Zpx : sprintf_s(tpc2,"%02X %02X     %s$%02X,X", TraceMem(Addy), TraceMem(Addy + 1), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1)); break;
		case Zpy : sprintf_s(tpc2,"%02X %02X     %s$%02X,Y", TraceMem(Addy), TraceMem(Addy + 1), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1)); break;
		case Idx : sprintf_s(tpc2,"%02X %02X     %s($%02X,X)", TraceMem(Addy), TraceMem(Addy + 1), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1)); break;
		case Idy : sprintf_s(tpc2,"%02X %02X     %s($%02X),Y", TraceMem(Addy), TraceMem(Addy + 1), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1)); break;
		case Rel :
			if (TraceMem(Addy + 1) & 0x80)
			{
				BYTE TpRelVal = ~TraceMem(Addy + 1) + 1;
	//			sprintf_s(tpc2,"%02X %02X     %s-$%02X", TraceMem(Addy), TraceMem(Addy + 1), TraceArr[TraceMem(Addy)], TpRelVal);	//Is this Right?...
				sprintf_s(tpc2,"%02X %02X     %s$%04X", TraceMem(Addy), TraceMem(Addy + 1), TraceArr[TraceMem(Addy)], Addy + 2 - TpRelVal);
			} else {
	//			sprintf_s(tpc2,"%02X %02X     %s$%02X", TraceMem(Addy), TraceMem(Addy + 1), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1));
				sprintf_s(tpc2,"%02X %02X     %s$%04X", TraceMem(Addy), TraceMem(Addy + 1), TraceArr[TraceMem(Addy)], Addy + 2 + TraceMem(Addy + 1));
			}
			break;
		};
...
void cNES_Debugger::Update()
{
	if (Logging)
	{
		sprintf_s(Out2,"A:%02X X:%02X Y:%02X P:%02X SP:%02X\n\0",CPU->A,CPU->X,CPU->Y,CPU->P,CPU->SP);
...
//		if (gData->Main->Stop)	//Only while stepping
		{
			...
			char tpc[40];
			sprintf_s(tpc,"PC: %04X",CPU->PC);		TextOut(RegDC, 1, 0, tpc, strlen(tpc));
			sprintf_s(tpc,"A: %02X",CPU->A);		TextOut(RegDC, 1, 1*FontHeight, tpc, strlen(tpc));
			sprintf_s(tpc,"X: %02X",CPU->X);		TextOut(RegDC, 1, 2*FontHeight, tpc, strlen(tpc));
			sprintf_s(tpc,"Y: %02X",CPU->Y);		TextOut(RegDC, 1, 3*FontHeight, tpc, strlen(tpc));
			sprintf_s(tpc,"SP: %02X",CPU->SP);		TextOut(RegDC, 1, 4*FontHeight, tpc, strlen(tpc));
			sprintf_s(tpc,"P: %02X", CPU->P);		TextOut(RegDC, 1, 5*FontHeight, tpc, strlen(tpc));
			sprintf_s(tpc,"->");
			if (CPU->P & FLAG_N) strcat_s(tpc,"N"); else strcat_s(tpc," ");
			if (CPU->P & FLAG_V) strcat_s(tpc,"V"); else strcat_s(tpc," ");
			strcat_s(tpc," ");
			if (CPU->P & FLAG_B) strcat_s(tpc,"B"); else strcat_s(tpc," ");
			if (CPU->P & FLAG_I) strcat_s(tpc,"I"); else strcat_s(tpc," ");
			if (CPU->P & FLAG_D) strcat_s(tpc,"D"); else strcat_s(tpc," ");
			if (CPU->P & FLAG_Z) strcat_s(tpc,"Z"); else strcat_s(tpc," ");
			if (CPU->P & FLAG_C) strcat_s(tpc,"C"); else strcat_s(tpc," ");
			TextOut(RegDC, 1, 6*FontHeight, tpc, strlen(tpc));

			if (PPU->SLnum < 240)
			{
				if (PPU->Xnum < 256)
					sprintf_s(tpc,"SLnum: %i, Pixel %i",PPU->SLnum, PPU->Xnum);
				else
					sprintf_s(tpc,"SLnum: %i, in HBlank",PPU->SLnum);
			} else {
				sprintf_s(tpc,"SLnum: %i, in VBlank", PPU->SLnum);
			}
			TextOut(RegDC, 1, 7*FontHeight, tpc, strlen(tpc));
			sprintf_s(tpc,"CPU Ticks: %i/%i", CPU->Clockticks, CPU->Clockticks / 12);		TextOut(RegDC, 1, 8*FontHeight, tpc, strlen(tpc));
			sprintf_s(tpc,"VRAMAddy: %04X", PPU->VRAMAddr);		TextOut(RegDC, 1, 9*FontHeight, tpc, strlen(tpc));

			int i;
			sprintf_s(tpc,"CPU Pages:");			TextOut(RegDC, 1, 12*FontHeight, tpc, strlen(tpc));
			for (i=0;i<8;i++)
			{ sprintf_s(tpc,"%02X",Map_GetPRG_ROM4(i+8));TextOut(RegDC, i*24, 13*FontHeight, tpc, strlen(tpc)); }
			sprintf_s(tpc,"PPU Pages:");			TextOut(RegDC, 1, 14*FontHeight, tpc, strlen(tpc));
			for (i=0;i<8;i++)
			{ sprintf_s(tpc,"%03X",Map_GetCHR_ROM1(i)); TextOut(RegDC, i*30, 15*FontHeight, tpc, strlen(tpc)); }
...
void cNES_Debugger::AddInst()
{
	if (Logging)
	{
		...
		sprintf_s(Out1, "%04X    %02X", Addy, TraceMem(Addy));				//PC
		
		char tpc2[30] = { 0 };						//Add Addy Mode Stuff

		switch (TraceAddyMode[TraceMem(Addy)])	
		{
		case ImmA : sprintf_s(tpc2," %02X      %s#$%02X      ", TraceMem(Addy + 1), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1)); break;
		case Abs : sprintf_s(tpc2," %02X %02X   %s$%04X     ", TraceMem(Addy + 1), TraceMem(Addy + 2), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1) | (TraceMem(Addy + 2) << 8)); break;
		case Zp : sprintf_s(tpc2," %02X      %s$%02X       ", TraceMem(Addy + 1), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1)); break;
		case Acc : sprintf_s(tpc2,"         %sA         ", TraceArr[TraceMem(Addy)]); break;
		case Imp : sprintf_s(tpc2,"         %s          ", TraceArr[TraceMem(Addy)]); break;
		case ERA : sprintf_s(tpc2,"         %s          ", TraceArr[TraceMem(Addy)]); break;
		//Add Comments for Addy pointed to/Actual Addy on These
		case Ind : sprintf_s(tpc2," %02X %02X   %s($%04X)   ", TraceMem(Addy + 1), TraceMem(Addy + 2), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1) | (TraceMem(Addy + 2) << 8)); break;
		case Absx : sprintf_s(tpc2," %02X %02X   %s$%04X,X   ", TraceMem(Addy + 1), TraceMem(Addy + 2), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1) | (TraceMem(Addy + 2) << 8)); break;
		case Absy : sprintf_s(tpc2," %02X %02X   %s$%04X,Y   ", TraceMem(Addy + 1), TraceMem(Addy + 2), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1) | (TraceMem(Addy + 2) << 8)); break;
		case Zpx : sprintf_s(tpc2," %02X      %s$%02X,X    ", TraceMem(Addy + 1), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1)); break;
		case Zpy : sprintf_s(tpc2," %02X      %s$%02X,Y    ", TraceMem(Addy + 1), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1)); break;
		case Idx : sprintf_s(tpc2," %02X      %s($%02X,X)   ", TraceMem(Addy + 1), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1)); break;
		case Idy : sprintf_s(tpc2," %02X      %s($%02X),Y   ", TraceMem(Addy + 1), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1)); break;
		case Rel :
			if (TraceMem(Addy + 1) & 0x80)
			{
				BYTE TpRelVal = ~TraceMem(Addy + 1) + 1;
				sprintf_s(tpc2," %02X      %s-$%02X      ", TraceMem(Addy + 1), TraceArr[TraceMem(Addy)], TpRelVal);	//Is this Right?...
			} else {
				sprintf_s(tpc2," %02X      %s$%02X       ", TraceMem(Addy + 1), TraceArr[TraceMem(Addy)], TraceMem(Addy + 1));
			}
			break;
		};
		strcat_s(Out1, tpc2);
