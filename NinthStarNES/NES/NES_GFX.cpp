...
void cNES_GFX::ReLoad(short Width, short Height, int NewMode)
{
	HDC tdc = GetWindowDC(GetDesktopWindow());
	Depth = (GetDeviceCaps(tdc,BITSPIXEL) >> 3);
	if ((Depth != 2) && (Depth != 4))
	...
	if (Mode3D)
	{
		...
		memcpy(g_Vertices, Vertices, sizeof(CUSTOMVERTEX)*6);
		memcpy(g_Vertices, Vertices, sizeof(CUSTOMVERTEX)*12);
...
void cNES_GFX::DrawScreen()
{
	...
	if (FPSCnt > FSkip)
	{
		...
		if (Mode3D)
		{
			...
			float rx = 0, ry = 3.14159265f/32, rz = 3.14159265f/64;
			float cenx = D3DPP.BackBufferWidth/2.0f, ceny = D3DPP.BackBufferHeight/2.0f, cenz = 1.0f;
			...
			for (int p=0;p<6;p++)
			{
				...
				tpy = g_Vertices[p].y * cos(rx) - g_Vertices[p].z * sin(rx);
				tpz = g_Vertices[p].z * cos(rx) + g_Vertices[p].y * sin(rx);

				g_Vertices[p].z = tpz * cos(ry) - g_Vertices[p].x * sin(ry) + cenz;
				tpx = g_Vertices[p].x * cos(ry) + tpz * sin(ry);
				
				g_Vertices[p].x = tpx * cos(rz) - tpy * sin(rz) + cenx;
				g_Vertices[p].y = tpy * cos(rz) + tpx * sin(rz) + ceny;
			}
